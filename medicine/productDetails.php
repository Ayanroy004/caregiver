...
if (isset($_GET['add_to_cart']) && isset($_GET['quantity'])) {
    ...
    $sql_check = "SELECT mitems FROM orders WHERE mid='$product_id'";
    $result_check = $conn->query($sql_check);

    if ($result_check->num_rows > 0) {
        // Update the existing order if the product already exists
        $row = $result_check->fetch_assoc();
        $current_quantity = intval($row['mitems']);
        $new_quantity = min($current_quantity + $quantity, 4); // Limit to 4 items

        if ($new_quantity > $current_quantity) {
            $sql_update = "UPDATE orders SET mitems='$new_quantity' WHERE mid='$product_id'";
            if ($conn->query($sql_update) === TRUE) {
                echo "<script>window.location.href='medicineCart.php';</script>";
            } else {
                echo "<script>alert('Error updating cart: " . $sql_update . "<br>" . $conn->error . "');</script>";
            }
        } else {
            echo "<script>alert('Maximum limit of 4 items reached.');</script>";
            echo "<script>window.location.href='medicineCart.php';</script>";
        }
    } else {
        // Insert new order if the product does not exist in the orders table
        $quantity = min($quantity, 4); // Limit to 4 items
        $sql_insert = "INSERT INTO orders (mid, mitems) VALUES ('$product_id', '$quantity')";
        if ($conn->query($sql_insert) === TRUE) {
            echo "<script>window.location.href='medicineCart.php';</script>";
        } else {
            echo "<script>alert('Error adding to cart: " . $sql_insert . "<br>" . $conn->error . "');</script>";
        }
    }
}

// Fetch product details from the database
$sql = "SELECT mname, mrp, image, description FROM products WHERE id='$product_id'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '
        <section class="sec-gap productDetails divider">
            ...
            <div class="product-details-wrapper">
                <div class="custom-container">
                    <div class="row">
                        ...
                        <path d="M42.883 0H3.12064C2.29363 -7.67422e-07 1.50044 0.438112 0.915314 1.21809C0.330186 1.99808 0.000975245 3.05614 0 4.15985L0 8.72734C0 9.83191 0.328781 10.8912 0.914014 11.6723C1.49925 12.4533 2.29299 12.8921 3.12064 12.8921H5.4096L8.832 19.375L12.2544 12.8921H42.8904C43.7158 12.8869 44.5062 12.446 45.0888 11.6657C45.6715 10.8853 45.999 9.82897 46 8.72734V4.15985C45.9942 3.05898 45.664 2.00538 45.0807 1.22692C44.4974 0.448464 43.7079 0.00771672 42.883 0Z" fill="#A02334"/>
                      </svg>
                                        </div>
                                ...
                                <div class="org-price">
                                    <div class="rupees">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="16" viewBox="0 0 12 16" fill="none">
                      <g clip-path="url(#clip0_137_622)">
                        <path d="M0 0.500043C0 0.500043 1.74264 0.500043 3.5 0.500043M8 16.5L1 9.50004C1 9.50004 2.5 9.50004 3.5 9.50004C4.5 9.50004 9 9.56183 9 5.00004C9 0.438253 4.5 0.500043 3.5 0.500043M12 0.500043C12 0.500043 6.42893 0.500043 3.5 0.500043M0 4.50004H12" stroke="#FFFFFF"/>
                      </g>
                      <defs>
                        <clipPath id="clip0_137_622">
                          <rect width="12" height="16" fill="white" />
